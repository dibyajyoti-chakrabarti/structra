...
### Cognito (fully managed)
Auth (email OTP + Google/GitHub OAuth) is the `modules/cognito` module, built from
scratch in this account. Terraform owns the **user pool**, the **public SPA app client**
(`structra-web`), both **identity providers** (Google native + GitHub via the OIDC shim, below),
the **hosted-UI domain** (`auth.structra.cloud`), and all **5 trigger Lambdas** (pre-signup,
...
> **Why a custom hosted-UI domain:** Cognito prefix domains are globally unique per region and
> the abandoned pool in the previous AWS account still holds `structra-auth`, which cannot be
> released without access to that account. `auth.structra.cloud` sits on a domain we control, so
> it is immune to that class of problem. The catch is that AWS will not create a custom domain
> until the parent domain has an A record, which makes a from-scratch build a two-phase apply.

> **Config notes:** the IdP `client_secret`s are `ignore_changes`d because Cognito never returns
> them on read and would otherwise show a perpetual diff. The `email` schema attribute is
> immutable once the pool exists, so changing it means replacing the pool. Trigger Lambda code is
> managed from the repo (`lambdas/*.py`).

> **GitHub OIDC shim:** GitHub is OAuth2, not OIDC, so Cognito can't federate it directly. The
> `modules/github-oidc-shim` module (API Gateway + 5 Lambdas) exposes the standard OIDC endpoints
> GitHub lacks; the Cognito GitHub IdP points its `oidc_issuer` at it. Source is vendored from
> `github-cognito-openid-wrapper` at `services/github-oidc-shim` and built by `make shim-build`.
>
> The RSA key that signs the `id_token` is generated by Terraform (`tls_private_key`) and passed
> to the Lambdas as environment config. Upstream inlines it into the webpack bundle instead, which
> means the deployed zip is the only copy of the key: the previous deployment was adopted that way
> from a CloudFormation stack, and when access to the old AWS account was lost the key went with
> it and the service could not be rebuilt. Keeping the key out of the artifact is what makes this
> module reproducible.
