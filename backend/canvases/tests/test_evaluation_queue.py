...
from canvases.evaluation_queue import LocalQueue
from canvases.evaluation_worker import process_next_job
...
class LocalQueueApiTests(EvaluationQueueFixtureMixin, APITestCase):
    def setUp(self):
        self.create_fixture()
...
class LocalQueueWorkerTests(EvaluationQueueFixtureMixin, TestCase):
    ...
    @patch('canvases.evaluation_worker.run_evaluation_job')
    ...
    @patch('canvases.evaluation_worker.run_evaluation_job', side_effect=RuntimeError('transient worker crash'))
